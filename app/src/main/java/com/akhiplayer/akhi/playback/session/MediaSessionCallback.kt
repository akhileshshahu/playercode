...
import com.akhiplayer.akhi.notification.PlaybackNotificationBuilder
import com.akhiplayer.akhi.playback.BecomingNoisyReceiver
...
import com.akhiplayer.akhi.utils.getAlbumArtBitmap
...
internal class MediaSessionCallback(
    ...
    private fun startPlayback(mediaId: String) {
        ...
        with(serviceContext) {
            startService(Intent(this, AkhiMediaService::class.java))
            with(trackPlayer) {
                reset()
                setDataSourceFromMediaId(serviceContext, mediaId)
                prepare()
